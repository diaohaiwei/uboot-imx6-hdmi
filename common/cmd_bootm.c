...
{
	trace("yusw enter boot-selected_os\n");
	arch_preboot_os();
	boot_fn(state, argc, argv, images);
	trace("yusw leave boot-selected_os\n");
		...
		int boot_progress)
{
	...
	if (states & BOOTM_STATE_START){
		trace("yusw BOOTM_STATE_START\n");
		ret = bootm_start(cmdtp, flag, argc, argv);
	}

	if (!ret && (states & BOOTM_STATE_FINDOS)){
		trace("yusw BOOTM_STATE_FINDOS\n");
		ret = bootm_find_os(cmdtp, flag, argc, argv);
	}

	if (!ret && (states & BOOTM_STATE_FINDOTHER)) {
		trace("yusw BOOTM_STATE_FINDOTHER\n");
	...
	if (!ret && (states & BOOTM_STATE_RAMDISK)) {
		...
		trace("yusw Relocate the ramdisk\n");
	...
	if (!ret && (states & BOOTM_STATE_OS_CMDLINE)){
		trace("yusw BOOTM_STATE_OS_CMDLINE\n");
		ret = boot_fn(BOOTM_STATE_OS_CMDLINE, argc, argv, images);
	}
	if (!ret && (states & BOOTM_STATE_OS_BD_T)){
		trace("yusw BOOTM_STATE_OS_BD_T\n");
		ret = boot_fn(BOOTM_STATE_OS_BD_T, argc, argv, images);
	}
	if (!ret && (states & BOOTM_STATE_OS_PREP)){
		trace("yusw BOOTM_STATE_OS_PREP\n");
		ret = boot_fn(BOOTM_STATE_OS_PREP, argc, argv, images);
	}

#ifdef CONFIG_TRACE
	puts("yusw CONFIG_TRACE\n");
			...
			char * const argv[], bootm_headers_t *images)
{
	...
	debug("yusw bootz_setup\n");
	ret = bootz_setup(images->ep, &zi_start, &zi_end);
	if (ret != 0)
		return 1;

	debug("yusw lmb_reserve\n");
	 ...
	 */
	debug("yusw bootm_find_ramdisk\n");
	if (bootm_find_ramdisk(flag, argc, argv))
		return 1;

#if defined(CONFIG_OF_LIBFDT)
	debug("yusw bootm_find_fdt\n");
	if (bootm_find_fdt(flag, argc, argv))
		return 1;
#endif

#ifdef CONFIG_SECURE_BOOT
	debug("yusw authenticate_image\n");
	extern uint32_t authenticate_image(
			uint32_t ddr_start, uint32_t image_size);
